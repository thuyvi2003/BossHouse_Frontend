...
export default function UserBookingForm({ onSuccess }) {
  ...
  const [form, setForm] = useState({
    ...
    pet_name: "",
    pet_species: "",
    services: selectedServiceFromState ? [{ ...selectedServiceFromState, quantity: 1 }] : [],
  ...
  // Tính tổng giá
  useEffect(() => {
    const total = form.services.reduce((sum, s) => sum + (s.base_price || 0) * s.quantity, 0);
  ...
  const handleSubmit = async e => {
    ...
    const payload = {
      ...
      pet_id: form.pet_id,
      services: form.services.map(s => ({ service_id: s._id, quantity: s.quantity })),
    ...
    try {
      ...
      console.error(err);
      setErrors({ form: err.response?.data?.message || "Failed to create booking" });
    }
  };
    ...
    <div className="bg-white p-6 rounded-xl shadow-lg max-w-3xl mx-auto mt-6">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input type="text" name="phone" value={form.phone} readOnly
                 className="w-full border rounded p-2 focus:ring-2 focus:ring-yellow-600 focus:border-transparent bg-gray-100 cursor-not-allowed" />
        </div>
        ...
        <div>
          ...
          <input type="text" name="pet_name" value={form.pet_name} readOnly
                 className="w-full border rounded p-2 focus:ring-2 focus:ring-yellow-600 focus:border-transparent bg-gray-100 cursor-not-allowed" />
        </div>
        ...
        <div>
          ...
          <select name="pet_species" value={form.pet_species} onChange={handleChange}
        ...
        <div className="col-span-1 md:col-span-2">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {form.services.map(s => (
              <div key={s._id} className="flex items-center gap-2 px-3 py-1 rounded-full shadow-sm text-yellow-800 bg-yellow-100">
                <span>{s.name} × {s.quantity}</span>
                <div className="flex gap-1">
                  <button type="button" onClick={() => handleAddService(s._id)}
                          className="w-6 h-6 flex items-center justify-center bg-green-500 text-white rounded-full">+</button>
                  <button type="button" onClick={() => handleRemoveService(s._id)}
                          className="w-6 h-6 flex items-center justify-center bg-red-500 text-white rounded-full">-</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="col-span-1 md:col-span-2">
          ...
          <div className="flex flex-wrap md:flex-nowrap gap-4 items-center">
            <div className="flex flex-col min-w-[150px] relative">
              ...
              />
              <Calendar className="absolute left-2 top-2 w-5 h-5 text-gray-500 cursor-pointer"
            ...
            <div className="flex flex-col min-w-[180px]">
              <div className="flex items-center gap-1">
                ...
                {/* Hour */}
                <select className="border rounded p-2 w-12"
                ...
                {/* Minute */}
                <select className="border rounded p-2 w-14"
                ...
                {/* AM/PM */}
                <select className="border rounded p-2 w-16"
